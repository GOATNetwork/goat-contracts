...
describe("RateLimiter", async () => {
    ...
    it("fail", async () => {
        const { rateLimiter } = await loadFixture(fixture)
        await expect(rateLimiter.fail1()).revertedWithCustomError(rateLimiter, "TooManyRequest")
        await expect(rateLimiter.fail2()).revertedWithCustomError(rateLimiter, "RateLimitExceeded")
        await expect(rateLimiter.fail3()).revertedWithCustomError(rateLimiter, "TooManyRequest")
        await expect(rateLimiter.fail4()).revertedWithCustomError(rateLimiter, "RateLimitExceeded")
    })
})
